...
internal fun List<MediaItem>.firstWithHref(href: String) = first {
    it.metadata?.href == href.takeWhile { it !in "#?" }
}

internal fun List<MediaItem>.indexOfFirstWithHref(href: String) = indexOfFirst {
    it.metadata?.href == href.takeWhile { it !in "#?" }
}
